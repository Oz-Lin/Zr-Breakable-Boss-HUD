...
public void OnClientCookiesCached(int client)
{
	...
	else	BossHudClientEnum[client].e_bIsHitEnabled = true;
...
void MenuClientBhud(int client)
{
	...
	char m_sBoss_HudTemp[16];
	char m_sBrekable_HudTemp[16];
	char m_sBoss_Hit_MarkerTemp[16];
	char m_sBoss_Hits_SoundTemp[16];
	char m_sBoss_Hits_ZombieTemp[16];

	if (BossHudClientEnum[client].e_bHpBhEnable) Format(m_sBoss_HudTemp ,sizeof(m_sBoss_HudTemp) ,"%t" ,"Enabled" );
	else Format(m_sBoss_HudTemp ,sizeof(m_sBoss_HudTemp) ,"%t" ,"Desabled" );

	if (BossHudClientEnum[client].e_bHpBrEnable) Format(m_sBrekable_HudTemp ,sizeof(m_sBrekable_HudTemp) ,"%t" ,"Enabled" );
	else Format(m_sBrekable_HudTemp ,sizeof(m_sBrekable_HudTemp) ,"%t" ,"Desabled" );

	if (BossHudClientEnum[client].e_bHitmEnable) Format(m_sBoss_Hit_MarkerTemp ,sizeof(m_sBoss_Hit_MarkerTemp) ,"%t" ,"Enabled" );
	else Format(m_sBoss_Hit_MarkerTemp ,sizeof(m_sBoss_Hit_MarkerTemp) ,"%t" ,"Desabled" );

	if (BossHudClientEnum[client].e_bHitmSound) Format(m_sBoss_Hits_SoundTemp ,sizeof(m_sBoss_Hits_SoundTemp) ,"%t" ,"Enabled" );
	else Format(m_sBoss_Hits_SoundTemp ,sizeof(m_sBoss_Hits_SoundTemp) ,"%t" ,"Desabled" );

	if (BossHudClientEnum[client].e_bIsHitEnabled) Format(m_sBoss_Hits_ZombieTemp ,sizeof(m_sBoss_Hits_ZombieTemp) ,"%t" ,"Enabled" );
	else Format(m_sBoss_Hits_ZombieTemp ,sizeof(m_sBoss_Hits_ZombieTemp) ,"%t" ,"Desabled" );
	...
	MenuBhud.AddItem("Boss_Hud_Enable", m_sBoss_Hud, MenuGetItemDraw(g_bBossHud));
	MenuBhud.AddItem("Brekable_Hud_Enable", m_sBrekable_Hud, MenuGetItemDraw(g_bBreakableHud));
	MenuBhud.AddItem("Boss_Hit_Marker_Enable", m_sBoss_Hit_Marker);
	MenuBhud.AddItem("Boss_Hits_Sound_Enable", m_sBoss_Hits_Sound);
	MenuBhud.AddItem("Boss_Hits_Zombie_Enable", m_sBoss_Hits_Zombie, MenuGetItemDraw(g_bDamgeHud));
...
public Action Timer_HUDUpdate(Handle timer) {
	...
	if (strlen(message) != 0) {
		for (int client = 1; client <= MaxClients; client++) {
			if (IsClientInGame(client) && BossHudClientEnum[client].e_bHpBhEnable) {
